...
class SystemCog(commands.Cog, name="System"):
    ...
    async def system_create(self, interaction: discord.Interaction, name: str):
        ...
        if existing:
            await interaction.followup.send("❌ You already have a system registered! Use `/system edit` to update it.", ephemeral=True)
            return
    ...
    @app_commands.describe(
        name="System name", tag="Short tag (e.g. [SYS])",
        description="System description", color="Hex color (e.g. #FF69B4)",
        avatar="URL to a system avatar image", privacy="Default privacy (public/private)"
    )
    async def system_edit(self, interaction: discord.Interaction,
                          name: str = None, tag: str = None, description: str = None,
                          color: str = None, avatar: str = None, privacy: str = None):
        ...
        updates = {}
        if name: updates['system_name'] = name
        if tag is not None: updates['system_tag'] = tag
        if description is not None: updates['description'] = description
        if color: updates['color'] = color
        if avatar is not None: updates['avatar_url'] = avatar
        if privacy in ('public', 'private'): updates['privacy_default'] = privacy
    ...
    async def system_delete(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                "This will permanently erase all members, front history, journal entries, polls, and notes.\n\n"
        ...
        class ConfirmView(discord.ui.View):
            ...
            async def confirm(self, btn_i: discord.Interaction, button: discord.ui.Button):
                ...
                await btn_i.edit_original_response(
                    embed=discord.Embed(title="System deleted.", color=discord.Color.greyple()), view=None
                )
        ...
        await interaction.followup.send(embed=embed, view=ConfirmView(self.bot.db, system['id']), ephemeral=True)
    ...
    async def set_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await interaction.followup.send(f"✅ Front notifications will now be posted in {channel.mention}", ephemeral=True)
    ...
    async def system_export(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send("✅ Here's your system data export! Keep this safe.", file=file, ephemeral=True)
