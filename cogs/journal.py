"""
Journal cog — write and manage journal entries.
"""
...
class JournalModal(discord.ui.Modal, title="Journal Entry"):
    entry_title = discord.ui.TextInput(label="Title (optional)", placeholder="Give this entry a title...", required=False, max_length=100)
    content = discord.ui.TextInput(label="Content", placeholder="Write your journal entry here...", style=discord.TextStyle.paragraph, max_length=2000, required=True)
    tags = discord.ui.TextInput(label="Tags (comma-separated, optional)", placeholder="e.g. happy, therapy, trauma", required=False, max_length=200)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=self.is_private)
        tags = [t.strip() for t in self.tags.value.split(',') if t.strip()] if self.tags.value else []
        kwargs = {'is_private': 1 if self.is_private else 0, 'tags': json.dumps(tags)}
        if self.entry_title.value: kwargs['title'] = self.entry_title.value
        if self.member: kwargs['member_id'] = self.member['id']

        entry_id = await self.db.create_journal_entry(self.system['id'], self.content.value, **kwargs)
        ...
        if self.member: embed.set_author(name=get_member_display(self.member))
        if tags: embed.add_field(name="Tags", value=', '.join(f'`{t}`' for t in tags))
        embed.set_footer(text=f"{'🔒 Private' if self.is_private else '🌐 Shared'} • ID: {entry_id}")
        await interaction.followup.send(embed=embed, ephemeral=self.is_private)
...
class JournalCog(commands.Cog, name="Journal"):
    ...
    @app_commands.describe(member="Which member is writing (optional)", private="Keep this entry private (default: True)")
    async def journal_write(self, interaction: discord.Interaction, member: str = None, private: bool = True):
        # Modal must be sent as the FIRST response — no defer here
        system = await self.bot.db.get_system(str(interaction.user.id), str(interaction.guild_id))
        if not system:
            await interaction.response.send_message("You don't have a system yet! Use `/system create` first. 💜", ephemeral=True)
            return
    ...
    @app_commands.describe(member="Filter by member", page="Page number")
    async def journal_list(self, interaction: discord.Interaction, member: str = None, page: int = 1):
        ...
        if not entries:
            await interaction.followup.send("No journal entries yet. Use `/journal write` to start.", ephemeral=True)
            return

        items, current_page, total_pages = paginate(list(entries), page, 10)
        embed = discord.Embed(title=f"📓 {system['system_name']} — Journal", color=parse_color(system['color']))

        for e in items:
            title = e['title'] or f"Entry #{e['id']}"
            preview = e['content'][:80] + ('...' if len(e['content']) > 80 else '')
            lock = '🔒' if e['is_private'] else '🌐'
            embed.add_field(name=f"{lock} {title} — {e['created_at'][:10]}", value=f"*{preview}*\nID: `{e['id']}`", inline=False)
    ...
    async def journal_view(self, interaction: discord.Interaction, entry_id: int):
        ...
        embed = discord.Embed(title=entry['title'] or f"Entry #{entry['id']}", description=entry['content'], color=parse_color(system['color']))
        if entry['member_id']:
            member = await self.bot.db.get_member(entry['member_id'])
            if member: embed.set_author(name=get_member_display(member))
        tags = json.loads(entry['tags'] or '[]')
        if tags: embed.add_field(name="Tags", value=', '.join(f'`{t}`' for t in tags))
        embed.set_footer(text=f"{'🔒 Private' if entry['is_private'] else '🌐 Shared'} • Written {entry['created_at'][:10]}")
        await interaction.followup.send(embed=embed, ephemeral=bool(entry['is_private']))
    ...
    async def journal_delete(self, interaction: discord.Interaction, entry_id: int):
        ...
        await interaction.followup.send(f"✅ Entry #{entry_id} deleted.", ephemeral=True)
    ...
    async def journal_search(self, interaction: discord.Interaction, query: str):
        ...
        q = query.lower()
        matches = [e for e in all_entries if q in (e['content'] or '').lower() or q in (e['title'] or '').lower() or q in (e['tags'] or '').lower()]

        if not matches:
            await interaction.followup.send(f"No entries found matching `{query}`.", ephemeral=True)
            return

        embed = discord.Embed(title=f"🔍 Search: '{query}'", color=parse_color(system['color']))
        for e in matches[:8]:
            title = e['title'] or f"Entry #{e['id']}"
            preview = e['content'][:60] + ('...' if len(e['content']) > 60 else '')
            embed.add_field(name=f"{title} ({e['created_at'][:10]})", value=f"*{preview}* | ID: `{e['id']}`", inline=False)
