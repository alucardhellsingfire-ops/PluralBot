"""
Members cog — add, view, edit, archive, and delete system members.
"""

import discord
from discord import app_commands
from discord.ext import commands
from utils.helpers import member_embed, require_system, paginate, get_member_display, parse_color
...
class MemberCog(commands.Cog, name="Members"):
    ...
    @app_commands.describe(
        name="Member's name", pronouns="Their pronouns (e.g. she/her)",
        description="A short description or bio", color="Hex color code (e.g. #FF69B4)",
        role="Their role in the system", avatar="URL to their avatar image",
        proxy_prefix="Proxy prefix (e.g. 'A:')", proxy_suffix="Proxy suffix (e.g. '-A')",
        birthday="Birthday (e.g. March 15)"
    )
    async def member_add(self, interaction: discord.Interaction, name: str,
                         pronouns: str = None, description: str = None, color: str = None,
                         role: str = None, avatar: str = None, proxy_prefix: str = None,
                         proxy_suffix: str = None, birthday: str = None):
    ...
    async def member_view(self, interaction: discord.Interaction, name: str):
        ...
        if not results:
            await interaction.followup.send(f"❌ No member found matching `{name}`.", ephemeral=True)
            return
    ...
    @app_commands.describe(
        name="Member name to edit", new_name="New name", display_name="Display name",
        pronouns="Pronouns", description="Bio/description", color="Hex color",
        role="Role in system", avatar="Avatar URL", proxy_prefix="Proxy prefix",
        proxy_suffix="Proxy suffix", birthday="Birthday"
    )
    async def member_edit(self, interaction: discord.Interaction, name: str,
                          new_name: str = None, display_name: str = None, pronouns: str = None,
                          description: str = None, color: str = None, role: str = None,
                          avatar: str = None, proxy_prefix: str = None,
                          proxy_suffix: str = None, birthday: str = None):
    ...
    async def member_list(self, interaction: discord.Interaction, page: int = 1, show_archived: bool = False):
        await interaction.response.defer()
        system = await require_system(interaction, self.bot.db)
        if not system:
            return
        ...
        if not all_members:
            await interaction.followup.send("No members yet! Use `/member add` to add your first headmate. 💜")
            return

        items, current_page, total_pages = paginate(all_members, page, 15)
        color = parse_color(system['color'])
        embed = discord.Embed(title=f"💜 {system['system_name']} — Members", color=color)

        lines = []
        for m in items:
            display = get_member_display(m)
            parts = [f"**{display}**"]
            if m['pronouns']: parts.append(f"({m['pronouns']})")
            if m['role']: parts.append(f"• *{m['role']}*")
            if m['is_archived']: parts.append("*(archived)*")
        ...
        class ConfirmView(discord.ui.View):
            ...
            async def confirm(self, btn_i: discord.Interaction, button: discord.ui.Button):
                ...
                await btn_i.edit_original_response(
                    embed=discord.Embed(title=f"{display} has been removed.", color=discord.Color.greyple()), view=None
                )
            ...
            async def cancel(self, btn_i: discord.Interaction, button: discord.ui.Button):
                await btn_i.response.defer(ephemeral=True)
                await btn_i.edit_original_response(
                    embed=discord.Embed(title="Deletion cancelled.", color=discord.Color.green()), view=None
                )
    ...
    async def member_archive(self, interaction: discord.Interaction, name: str, unarchive: bool = False):
        await interaction.response.defer(ephemeral=True)
        system = await require_system(interaction, self.bot.db)
        if not system:
            return
        ...
        await self.bot.db.update_member(member['id'], is_archived=0 if unarchive else 1)
        action = "restored" if unarchive else "archived"
        await interaction.followup.send(f"✅ **{get_member_display(member)}** has been {action}.", ephemeral=True)
    ...
    async def member_note(self, interaction: discord.Interaction, name: str, note: str):
        ...
        results = await self.bot.db.search_member(system['id'], name)
        if not results:
            await interaction.followup.send(f"❌ Member `{name}` not found.", ephemeral=True)
            return

        member = results[0]
        await self.bot.db.add_note(system['id'], note, member['id'])
        await interaction.followup.send(f"✅ Note added for **{get_member_display(member)}**.", ephemeral=True)
    ...
    async def member_notes(self, interaction: discord.Interaction, name: str):
        ...
        notes = await self.bot.db.get_notes(system['id'], member['id'])
        if not notes:
            await interaction.followup.send(f"No notes found for **{get_member_display(member)}**.", ephemeral=True)
            return

        embed = discord.Embed(title=f"📝 Notes for {get_member_display(member)}", color=parse_color(member['color']))
        for n in notes[-10:]:
            embed.add_field(name=n['created_at'][:10], value=n['content'][:500], inline=False)
        await interaction.followup.send(embed=embed, ephemeral=True)
    ...
    async def group_create(self, interaction: discord.Interaction, name: str, description: str = None, color: str = None):
        await interaction.response.defer()
        system = await require_system(interaction, self.bot.db)
        if not system:
            return
        ...
        await interaction.followup.send(f"✅ Group **{name}** created! (ID: `{gid}`)")
    ...
    async def group_add_member(self, interaction: discord.Interaction, group_name: str, member_name: str):
        ...
        if not results:
            await interaction.followup.send(f"❌ Member `{member_name}` not found.", ephemeral=True)
            return

        await self.bot.db.add_member_to_group(group['id'], results[0]['id'])
        await interaction.followup.send(f"✅ **{get_member_display(results[0])}** added to group **{group['name']}**.")
    ...
    async def group_list(self, interaction: discord.Interaction):
        ...
        if not groups:
            await interaction.followup.send("No groups yet! Use `/group create` to make one.")
            return

        embed = discord.Embed(title=f"🗂️ {system['system_name']} — Groups", color=parse_color(system['color']))
        for g in groups:
            members = await self.bot.db.get_group_members(g['id'])
            names = ', '.join(get_member_display(m) for m in members) if members else '*Empty*'
            embed.add_field(name=f"**{g['name']}**", value=f"{g['description'] or ''}\nMembers: {names}", inline=False)
        await interaction.followup.send(embed=embed)
    ...
    async def group_delete(self, interaction: discord.Interaction, name: str):
        ...
        if not group:
            await interaction.followup.send(f"❌ Group `{name}` not found.", ephemeral=True)
            return

        await self.bot.db.delete_group(group['id'])
        await interaction.followup.send(f"✅ Group **{name}** deleted.", ephemeral=True)
