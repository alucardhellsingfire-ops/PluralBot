...
Commands: /friends add, accept, list, remove, front
"""
...
class FriendsCog(commands.Cog, name="Friends"):
    """Connect with other systems and see who's fronting."""
    ...
    async def friends_add(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not their_system:
            await interaction.response.send_message(
                f"❌ {user.display_name} doesn't have a system registered in this server.",
                ephemeral=True
            )
            return
        ...
        if not success:
            await interaction.response.send_message(
                "❌ You've already sent a request or are already friends.", ephemeral=True
            )
            return
        ...
        try:
            embed = discord.Embed(
                ...
                description=(
                    f"**{system['system_name']}** ({interaction.user.mention}) "
                    f"sent you a system friend request!\n\n"
                    f"Use `/friends accept @{interaction.user.display_name}` to accept."
                ),
    ...
    async def friends_accept(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not their_system:
            await interaction.response.send_message(
                f"❌ {user.display_name} doesn't have a system here.",
                ephemeral=True
            )
            return
        ...
        if not req:
            await interaction.response.send_message(
                f"❌ No pending friend request from {user.display_name}.",
                ephemeral=True
            )
            return
    ...
    async def friends_pending(self, interaction: discord.Interaction):
        ...
        pending = await self.bot.db.get_pending_requests(system['id'])
        if not pending:
            await interaction.response.send_message("No pending friend requests.", ephemeral=True)
            return
        ...
        for req in pending:
            embed.add_field(
                ...
                value=f"From <@{req['user_id']}> • Sent {req['created_at'][:10]}",
                inline=False
            )
    ...
    async def friends_list(self, interaction: discord.Interaction):
        ...
        if not friends:
            await interaction.response.send_message(
                "No friends yet! Use `/friends add` to connect with other systems.", ephemeral=True
            )
            return

        color = parse_color(system['color'])
        embed = discord.Embed(
            title=f"💜 {system['system_name']} — Friends",
            color=color
        )
        for f in friends:
            embed.add_field(
                name=f['system_name'],
                value=f"<@{f['user_id']}> • Since {f['created_at'][:10]}",
                inline=False
            )
    ...
    async def friends_front(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not their_system:
            await interaction.response.send_message(
                f"❌ {user.display_name} doesn't have a registered system here.",
                ephemeral=True
            )
            return
        ...
        if not any(f['friend_system_id'] == their_system['id'] for f in friends):
            await interaction.response.send_message(
                f"❌ You're not friends with **{their_system['system_name']}** yet.",
                ephemeral=True
            )
            return
        ...
        if their_system['privacy_default'] == 'private':
            await interaction.response.send_message(
                f"🔒 **{their_system['system_name']}** has their system set to private.",
                ephemeral=True
            )
            return
    ...
    async def friends_remove(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not their_system:
            await interaction.response.send_message(
                f"❌ {user.display_name} doesn't have a system here.", ephemeral=True
            )
            return

        await self.bot.db.remove_friend(system['id'], their_system['id'])
        await interaction.response.send_message(
            f"✅ Removed **{their_system['system_name']}** from your friends list.",
            ephemeral=True
        )
